...
import {Pagination} from "antd";
import headerBg from "@/assets/bg/board/ir_header_bg.png";
    ...
    useEffect(() => {
        const getEvent = async () => {
            try {
                ...
                await UserAPIService.getBoardList(21, page).then((result) => {
    ...
    return (
        <div>
            ...
            <div className={`boardHeaderWrap`}>
                <p className={`section-title ${styles.sectionTitle}`}>주요IR 일정</p>
                ...
                <table className='tableWrap notice'>
                    ...
                    <tbody>
                    {
                        listData?.list.map((itm, idx) => {
                            return (
                                <tr key={idx} className={itm.topFlag ? `sticky` : ''} onClick={()=>router.push('/board/ir/'+itm.uid)}>
