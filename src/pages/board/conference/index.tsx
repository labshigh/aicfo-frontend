...
import {Pagination} from "antd";
import headerBg from "@/assets/bg/board/conference header_bg.png";
    ...
    useEffect(() => {
        const getEvent = async () => {
            try {
                ...
                await UserAPIService.getBoardList(23, page).then((result) => {
    ...
    return (
        <div>
            ...
            <div className={`boardHeaderWrap`}>
                <p className={`section-title ${styles.sectionTitle}`}>국내-해외전시</p>
                ...
                <table className='tableWrap notice'>
                    ...
                    <tbody>
                    {
                        listData?.list.map((itm, idx) => {
                            return (
                                <tr key={idx} className={itm.topFlag ? `sticky` : ''} onClick={()=>router.push('/board/conference/'+itm.uid)}>
