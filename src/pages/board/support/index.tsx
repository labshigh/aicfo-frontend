...
import {Pagination} from "antd";
import headerBg from "@/assets/bg/board/support header_bg.png";
    ...
    useEffect(() => {
        const getEvent = async () => {
            try {
                ...
                await UserAPIService.getBoardList(22, page).then((result) => {
    ...
    return (
        <div>
            ...
            <div className={`boardHeaderWrap`}>
                <p className={`section-title ${styles.sectionTitle}`}>정부 지원 자금</p>
                ...
                <table className='tableWrap notice'>
                    ...
                    <tbody>
                    {
                        listData?.list.map((itm, idx) => {
                            return (
                                <tr key={idx} className={itm.topFlag ? `sticky` : ''} onClick={()=>router.push('/board/support/'+itm.uid)}>
